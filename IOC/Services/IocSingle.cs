...
public class IocSingle
{
    ...
    public static IServiceScopeFactory GetServiceScopeFactory() => ServiceScopeFactory;
    
    private IServiceProvider ServiceProvider{ set ; get; }
    
    private static ConcurrentDictionary<Type, object> scopeServicePool = new ConcurrentDictionary<Type, object>();
    
    public IocSingle(IServiceProvider serviceProvider,IServiceScopeFactory serviceScopeFactory,IConfiguration configuration)
    {
    ...
    {
        object? result = null;
        if (scopeServicePool.ContainsKey(typeof(T)))
        {
            scopeServicePool.TryGetValue(typeof(T), out result);
        }
        else
        {
            using (var scope = ServiceScopeFactory.CreateScope())
            {
                result = scope.ServiceProvider.GetService<T>();
                scopeServicePool.TryAdd(typeof(T), result);
            }
        }
        if(result == null) throw new Exception("未找到服务: " + typeof(T));
        return result as T;
    }
    ...
    public void Autowired(object service)
    {
        ...
        else
        {
            ...
            var GetService =
                typeof(IocSingle).GetMethod("GetService", BindingFlags.Static | BindingFlags.NonPublic);
